...
-- =============================================
CREATE PROCEDURE [dbo].[EvaluateEquipRates-Unmatched-InsertDetail] 
...
INSERT INTO [RA - Audit Results - Detail] ( Audit_Results_Detail_Types_of_Issues, Audit_Results_Total_Detail_Error_Cost, Audit_Results_Total_Detail_Cost, Audit_Results_Detail_Check_Performed, Audit_Results_Detail_Check_Status_ID, Audit_Results_Count_of_Failures, Audit_Results_Sub_Cat_Type_ID, Audit_Results_Detail_Type_ID, Audit_Results_Category_Type_ID, Rest_Bill_ID, Audit_Summary_ID, Audit_Results_Total_Detail_Bill_Amt )
SELECT 'Equipment Not in Industry Table: '+[RA - Rest Bills Unscrubbed-Equipment].[Equipment_Name] AS Expr1, Sum([RA - Rest Bills Unscrubbed-Equipment].Total_Cost) AS SumOfTotal_Cost1, Sum([RA - Rest Bills Unscrubbed-Equipment].Total_Cost) AS SumOfTotal_Cost, 'Y' AS Expr7, 3 AS Expr6, 1 AS Expr2, 10 AS Expr4, 20 AS Expr5, 4 AS Expr3, [RA - Rest Bills Unscrubbed-Equipment].Rest_Bill_ID, @Summary_ID AS Expr8, Sum([RA - Rest Bills Unscrubbed-Equipment].Total_Cost) AS SumOfTotal_Cost2
FROM [RA - Rest Bills Unscrubbed-Equipment] LEFT JOIN [RA - Industry - Equip Range plus Alias - Max Rate] ON [RA - Rest Bills Unscrubbed-Equipment].Equipment_Name = [RA - Industry - Equip Range plus Alias - Max Rate].Industry_Equip_Name_Alias
GROUP BY [RA - Rest Bills Unscrubbed-Equipment].Rest_Bill_ID, [RA - Rest Bills Unscrubbed-Equipment].Job_ID, [RA - Rest Bills Unscrubbed-Equipment].Rest_Co_ID, [RA - Industry - Equip Range plus Alias - Max Rate].Industry_Equip_Name_Alias, [RA - Rest Bills Unscrubbed-Equipment].Equipment_Name
HAVING ((([RA - Rest Bills Unscrubbed-Equipment].Rest_Bill_ID)=@Bill_ID) AND (([RA - Rest Bills Unscrubbed-Equipment].Job_ID)=@Job_ID) AND (([RA - Rest Bills Unscrubbed-Equipment].Rest_Co_ID)=@Rest_Co_ID) AND (([RA - Industry - Equip Range plus Alias - Max Rate].Industry_Equip_Name_Alias) Is Null));
