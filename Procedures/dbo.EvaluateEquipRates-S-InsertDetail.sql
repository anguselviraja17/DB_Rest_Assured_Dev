...
-- =============================================
CREATE PROCEDURE [dbo].[EvaluateEquipRates-S-InsertDetail] 
...
INSERT INTO [RA - Audit Results - Detail] ( Audit_Results_Detail_Types_of_Issues, Audit_Results_Total_Detail_Cost, Audit_Results_Detail_Check_Performed, Audit_Results_Detail_Check_Status_ID, Audit_Results_Count_of_Successes, Audit_Results_Sub_Cat_Type_ID, Audit_Results_Detail_Type_ID, Audit_Results_Category_Type_ID, Rest_Bill_ID, Audit_Summary_ID, Audit_Results_Total_Detail_Bill_Amt, Industry_High_Rate, Original_Rate, Original_Volume )
SELECT 'Equip found-Rate is not higher than Industry Range:  '+[RA - Rest Bills Unscrubbed-Equipment].[Equipment_Name] AS Expr1,
Sum([RA - Rest Bills Unscrubbed-Equipment].Total_Cost) AS SumOfTotal_Cost1,
'Y' AS Expr7,
1 AS Expr6,
1 AS Expr2,
11 AS Expr4,
...
@Summary_ID AS Expr8,
Sum([RA - Rest Bills Unscrubbed-Equipment].Total_Cost) AS SumOfTotal_Cost2,
[RA - Industry - Equip Range plus Alias - Max Rate].Quart3OfEquipment_High_Rate_Day*@AllowablePercentOver,
[RA - Rest Bills Unscrubbed-Equipment].Unit_Cost,
Sum([RA - Rest Bills Unscrubbed-Equipment].Quantity) AS SumOfQuantity
FROM
[RA - Rest Bills Unscrubbed-Equipment] INNER JOIN
[RA - Industry - Equip Range plus Alias - Max Rate] 
ON [RA - Rest Bills Unscrubbed-Equipment].Equipment_Name = [RA - Industry - Equip Range plus Alias - Max Rate].Industry_Equip_Name_Alias
GROUP BY
[RA - Rest Bills Unscrubbed-Equipment].Rest_Bill_ID,
[RA - Industry - Equip Range plus Alias - Max Rate].Quart3OfEquipment_High_Rate_Day*@AllowablePercentOver,
[RA - Rest Bills Unscrubbed-Equipment].Unit_Cost,
...
IIf((([Unit_Cost]-([Quart3OfEquipment_High_Rate_Day]*@AllowablePercentOver)))>0,'F','S')
HAVING (((
[RA - Rest Bills Unscrubbed-Equipment].Rest_Bill_ID)=@Bill_ID) AND
(([RA - Rest Bills Unscrubbed-Equipment].Job_ID)=@Job_ID) AND
(([RA - Rest Bills Unscrubbed-Equipment].Equipment_Name) Is Not Null) AND
((IIf((([Unit_Cost]-([Quart3OfEquipment_High_Rate_Day]*@AllowablePercentOver)))>0,'F','S'))='S'));
END
GO
